'''
Controller - for use with BeatSync
...
class Controller:
    ...
    def sample(self, t_tempo, IBD:InputBeatDetector, MBD:MachineBeatDetector):
        '''
        ...
        # Get parameters from InputBeatDetectoror and MachineBeatDetector
        tempo_p, BP_p0 = IBD.getBeatPositionFunction()
        tempo_m, BP_m0 = MBD.getBeatPositionFunction()
        ...
        if not MBD.isAlreadyShifted:
            ...
            error = MBD.getBarBeatPosition(t_tempo) - IBD.getBarBeatPosition(t_tempo)
            self.BP_errors.append(error)
            if (0 < error < self.Nb/2) or (-self.Nb/2 <= error < 0):
                BP_m0_d = IBD.getBeatPosition(t_tempo) + error - tempo_m*t_tempo
            elif error >= self.Nb/2:
                BP_m0_d = IBD.getBeatPosition(t_tempo) + error - self.Nb - tempo_m*t_tempo
            else:   # error < -Nb/2
                BP_m0_d = IBD.getBeatPosition(t_tempo) + error + self.Nb - tempo_m*t_tempo
            ...
            if BP_shift != 0:   # True when no shift required - we expect this if machine starts with input (e.g drummer)
                MBD.shiftBeatPosition(BP_shift)
            tempo_m, BP_m0 = MBD.getBeatPositionFunction()            

        

        # 1. Check if ahead or behind or within acceptable error
        BP_error = MBD.getBeatPosition(t_tempo) - IBD.getBeatPosition(t_tempo)
